...
class KiwoomAPI:
    ...
    def _on_receive_tr_data(self, screen_no, rqname, trcode, record_name, next,
        ...
        # opt10030: 거래량 상위
            ...
            rows = []
            repeat_cnt = self.get_repeat_cnt(trcode, rqname)
            self._debug(f"[TR] opt10030 repeat_cnt={repeat_cnt} (rqname={rqname})")
            ...
            self.tr_data["opt10030_rows"] = rows
            self._debug(f"[TR] opt10030 extracted {len(rows)} rows")
    ...
    def get_volume_top_opt10030(self, params: dict, prev_next=0):
        ...
        Args:
            params: 키움 매뉴얼 입력값 dict (예: {"시장구분": "000", ...})
            prev_next: 다음 페이지 조회 여부 (0/2 또는 "Y")
        Returns:
            dict: {"rows": list[dict], "next": str}
        """
        params = params or {}
        for k, v in params.items():
            self.set_input_value(k, str(v))

        next_flag = 0
        if str(prev_next).strip().upper() in ("Y", "2", "TRUE", "T", "1"):
            next_flag = 2

        self.comm_rq_data("거래량상위", "opt10030", next_flag, "0106")

        rows = self.tr_data.get("opt10030_rows", [])
        next_yn = self.tr_data.get("next", "")
        return {"rows": rows, "next": next_yn}
    ...
    def _on_receive_real_data(self, code, real_type, real_data):
        """실시간 데이터 수신 이벤트"""
        if real_type == "주식체결":
            price = abs(int(self.get_comm_real_data(code, 10) or 0))   # 현재가
            cum_vol = abs(int(self.get_comm_real_data(code, 13) or 0)) # 당일 누적거래량 (FID 13)

            # NXT 코드(_NX) → base code 변환
            base_code = from_nxt_code(code) if is_nxt_code(code) else code

            if self.on_real_data_callback:
                self.on_real_data_callback(base_code, price, cum_vol)
